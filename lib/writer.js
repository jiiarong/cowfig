...
function write (data, fname, opt) {
  ...
  if (data.__copy) {
    for (let j = 0; j < data.__copy.length; j++) {
      destFile = opt.destBase + path.dirname(fname) + path.sep + (data.__copy[j].dest || path.basename(data.__copy[j].src));
  ...
  if ((Object.getOwnPropertyNames(data).length == 0) && (!writerOpt.emptyObj))
    console.log('%sskip empty file%s: %j\r', YELLOW, NORMAL, destFile);
  // Check if overwrite exist destination file
    ...
    console.log('%sskip newer file%s: %j\r', YELLOW, NORMAL, destFile);
  else
